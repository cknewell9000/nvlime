function! TestSimpleSendCB()
    function! s:TestSimpleSendCBDummyCB(conn, msg)
        let b:nvlime_test_send_cb_called_with_conn = a:conn
        let b:nvlime_test_send_cb_called_with_msg = a:msg
    endfunction

    let conn = nvlime#New()
    let b:nvlime_test_send_cb_called_with_conn = v:null
    let b:nvlime_test_send_cb_called_with_msg = v:null
    call nvlime#SimpleSendCB(conn, function('s:TestSimpleSendCBDummyCB'),
                \ 'TestSimpleSendCB', v:null, s:OKReturn('reply'))
    call assert_equal(conn, b:nvlime_test_send_cb_called_with_conn)
    call assert_equal('reply', b:nvlime_test_send_cb_called_with_msg)
    unlet b:nvlime_test_send_cb_called_with_conn
    unlet b:nvlime_test_send_cb_called_with_msg
endfunction
...
function! TestSetCurrentPackage()
    function! s:DummyPackageSetter(pkg, ...) dict
        let b:nvlime_test_dummy_package = a:pkg
    endfunction
    let ui = nvlime#ui#New()
    let ui['SetCurrentPackage'] = function('s:DummyPackageSetter')
    let conn = nvlime#New(v:null, ui)
    let b:nvlime_test_dummy_package = v:null
    call conn.SetCurrentPackage(['DUMMY-PACKAGE-2', 'DUMMY-PACKAGE-2'])
    call assert_equal(['DUMMY-PACKAGE-2', 'DUMMY-PACKAGE-2'], b:nvlime_test_dummy_package)
    unlet b:nvlime_test_dummy_package
endfunction
...
function! TestSetCurrentThread()
    function! s:DummyThreadSetter(thread, ...) dict
        let b:nvlime_test_dummy_thread = a:thread
    endfunction
    let ui = nvlime#ui#New()
    let ui['SetCurrentThread'] = function('s:DummyThreadSetter')
    let conn = nvlime#New(v:null, ui)
    let b:nvlime_test_dummy_thread = v:null
    call conn.SetCurrentThread({'name': 'DUMMY-THREAD', 'package': 'KEYWORD'})
    call assert_equal({'name': 'DUMMY-THREAD', 'package': 'KEYWORD'}, b:nvlime_test_dummy_thread)
    unlet b:nvlime_test_dummy_thread
endfunction

function! TestWithThread()
    function! s:DummyThreadGetter(...) dict
        return b:nvlime_test_dummy_thread
    endfunction
    ...
    function! s:DummyAction(conn)
        let b:nvlime_test_dummy_action_result = a:conn.GetCurrentThread()
    endfunction
    ...
    let ui['SetCurrentThread'] = function('s:DummyThreadSetter')
    let conn = nvlime#New(v:null, ui)
    let b:nvlime_test_dummy_thread = {'name': 'OLD-THREAD', 'package': 'KEYWORD'}
    let b:nvlime_test_dummy_action_result = v:null
    call conn.WithThread(1, function('s:DummyAction', [conn]))
    call assert_equal(1, b:nvlime_test_dummy_action_result)
    call assert_equal({'name': 'OLD-THREAD', 'package': 'KEYWORD'}, b:nvlime_test_dummy_thread)
    unlet b:nvlime_test_dummy_thread
    unlet b:nvlime_test_dummy_action_result
endfunction

function! TestWithPackage()
    function! s:DummyPackageGetter(...) dict
        return b:nvlime_test_dummy_package
    endfunction
    ...
    function! s:DummyAction(conn)
        let b:nvlime_test_dummy_action_result = a:conn.GetCurrentPackage()
    endfunction
    ...
    let ui['SetCurrentPackage'] = function('s:DummyPackageSetter')
    let conn = nvlime#New(v:null, ui)
    let b:nvlime_test_dummy_package = ['OLD-PKG', 'OLD-PKG']
    let b:nvlime_test_dummy_action_result = v:null
    call conn.WithPackage('NEW-PKG', function('s:DummyAction', [conn]))
    call assert_equal(['NEW-PKG','NEW-PKG'], b:nvlime_test_dummy_action_result)
    call assert_equal(['OLD-PKG', 'OLD-PKG'], b:nvlime_test_dummy_package)
    unlet b:nvlime_test_dummy_package
    unlet b:nvlime_test_dummy_action_result
endfunction
...
function! TestToRawForm()
    ...
    call assert_equal([['cons', '|some sym|', '', cursor_marker], 17, v:false],
                \ nvlime#ToRawForm('(cons |some sym| '))
    ...
    call assert_equal([['cons', ['list', '1', '2', '', cursor_marker]], 24, v:false],
                \ nvlime#ToRawForm("(cons ; abc\n  (list 1 2 "))
endfunction

function! TestMemoize()
    call assert_equal(9, nvlime#Memoize(function('abs', [-9]), -9, 'dummy_cache'))
    call assert_equal({-9: 9}, b:dummy_cache)

    let b:dummy_cache[-9] = 10
    call assert_equal(10, nvlime#Memoize(function('abs', [-9]), -9, 'dummy_cache'))

    call assert_equal(11, nvlime#Memoize(function('abs', [11]), 11, 'dummy_cache'))
    call assert_equal(2, len(b:dummy_cache))

    call assert_equal(12, nvlime#Memoize(function('abs', [12]), 12, 'dummy_cache'))
    call assert_equal(3, len(b:dummy_cache))

    call assert_equal(13, nvlime#Memoize(function('abs', [13]), 13, 'dummy_cache', b:, 1))
...
function! TestMessage(name, expected, reply, ...)
    function! s:TestMessageDummySend(dummy_conn, dummy_reply, msg, ...) dict
        ...
        let b:nvlime_test_dummy_sent_msg = a:msg
        if a:0 == 1
    ...
    let conn['CreateREPL'] = function('nvlime#contrib#repl#CreateREPL')
    let conn['ListenerEval'] = function('nvlime#contrib#repl#ListenerEval')
    let conn['FuzzyCompletions'] = function('nvlime#contrib#fuzzy#FuzzyCompletions')
    let conn['Autodoc'] = function('nvlime#contrib#arglists#Autodoc')
    let conn['InspectPresentation'] = function('nvlime#contrib#presentations#InspectPresentation')

    let conn['ClearTraceTree'] = function('nvlime#contrib#trace_dialog#ClearTraceTree')
    let conn['DialogToggleTrace'] = function('nvlime#contrib#trace_dialog#DialogToggleTrace')
    let conn['DialogTrace'] = function('nvlime#contrib#trace_dialog#DialogTrace')
    let conn['DialogUntrace'] = function('nvlime#contrib#trace_dialog#DialogUntrace')
    let conn['DialogUntraceAll'] = function('nvlime#contrib#trace_dialog#DialogUntraceAll')
    let conn['FindTrace'] = function('nvlime#contrib#trace_dialog#FindTrace')
    let conn['FindTracePart'] = function('nvlime#contrib#trace_dialog#FindTracePart')
    let conn['InspectTracePart'] = function('nvlime#contrib#trace_dialog#InspectTracePart')
    let conn['ReportPartialTree'] = function('nvlime#contrib#trace_dialog#ReportPartialTree')
    let conn['ReportSpecs'] = function('nvlime#contrib#trace_dialog#ReportSpecs')
    let conn['ReportTotal'] = function('nvlime#contrib#trace_dialog#ReportTotal')
    let conn['ReportTraceDetail'] = function('nvlime#contrib#trace_dialog#ReportTraceDetail')

    let conn['CreateMREPL'] = function('nvlime#contrib#mrepl#CreateMREPL')

    let ToCall = function(conn[a:name], a:000, conn)
    let b:nvlime_test_dummy_sent_msg = v:null
    call ToCall()
    call assert_equal(a:expected, b:nvlime_test_dummy_sent_msg)
    unlet b:nvlime_test_dummy_sent_msg
    return conn
endfunction
...
let b:messages_to_test = [
            ...
            \ ['ListenerEval',
                \ s:ExpectedEmacsRex('SWANK-REPL', 'LISTENER-EVAL', 'expression',
                    \ s:KW('WINDOW-WIDTH'), 80),
                \ s:OKReturn(['COMMON-LISP-USER', 'CL-USER']),
                \ 'expression', v:null, 80],
            \ ['Interrupt',
            ...
            \ ['FrameSourceLocation',
                ...
                \ 0],
            \ ['EvalStringInFrame',
                \ s:ExpectedEmacsRex('SWANK', 'EVAL-STRING-IN-FRAME',
                    \ 'expression', 0, 'DUMMY-PACKAGE', 6, 80),
                \ s:OKReturn(v:null),
                \ 'expression', 0, 'DUMMY-PACKAGE', v:null, 6, 80],
            \ ['InitInspector',
